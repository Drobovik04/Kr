...
<CardsAuthor v-bind:items="filteredArtists"/>
</div>
</template>

<script>
import bd from './bd.json';
import CardsAuthor from './CardsAuthor.vue';
        ...
        filteredArtists() {
            ...
            this.artists.forEach(artist => {
                const filteredPictures = artist.artist.pictures.filter(picture => {
                    return (
                        (this.selectedGenres.length === 0 || this.selectedGenres.every(genre => picture.genres.includes(genre))) &&
                ...
                // Если у художника есть хотя бы одна картина, соответствующая фильтрам, добавляем его в отфильтрованный список
                    filtered.push({
                        name: artist.artist.name,
                        path: artist.artist.path
                    });
                }
            });
            return filtered;
